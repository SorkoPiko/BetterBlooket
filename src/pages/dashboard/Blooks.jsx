...
function Blooks() {
    ...
    return (<Sidebar>
        <div id="selected">
            ...
            <img id="selectedBlook" data-locked={(!blooks[selected] && !packs[allBlooks[selected]?.set]?.free)} style={(!blooks[selected] && !packs[allBlooks[selected]?.set]?.free) ? { opacity: 0.75 } : {}} src={allBlooks[selected]?.mediaUrl} alt={selected} />
            <div id="blookName">{(!blooks[selected] && !packs[allBlooks[selected]?.set]?.free) ? "???" : selected}</div>
                        ...
                        <div id="packUnlocksWrapper">
                            {Object.keys((showHidden ? packs[pack]?.allBlooks : packs[pack]?.blooks) || {}).map((blook, i) => {
                                ...
                                return (<div className="packBlook" key={blook} data-place={i} style={{
                                    ...
                                    <img data-locked={!blooks[blook] && !packs[allBlooks[blook]?.set]?.free} onClick={() => setIndex(ind => ind + (i - selectedIndex))} src={allBlooks[blook]?.mediaUrl} alt={blook} />
